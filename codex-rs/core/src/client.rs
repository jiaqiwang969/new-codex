...
impl Clone for ModelClientState {
    fn clone(&self) -> Self {
        Self {
            ...
            responses_websocket_version: self.responses_websocket_version,
...
impl ModelClient {
    ...
    pub fn pre_establish_connection(&self, otel_manager: OtelManager, cwd: PathBuf) {
        if !self.state.provider.supports_websockets
            || self.state.responses_websocket_version.is_none()
    ...
    pub async fn preconnect(
        ...
        let ws_version = match self.state.responses_websocket_version {
            Some(v)
                if self.state.provider.supports_websockets
                    && !self.state.disable_websockets.load(Ordering::Relaxed) =>
            {
