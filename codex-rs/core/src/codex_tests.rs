...
async fn session_configuration_apply_preserves_split_file_system_policy_on_cwd_only_update() {
    ...
    let (updated, _) = session_configuration
...
#[tokio::test]
async fn session_configuration_apply_keeps_runtime_provider_override_when_provider_id_matches_original()
 {
    let mut session_configuration = make_session_configuration_for_tests().await;
    let runtime_base_url = "http://127.0.0.1:4010/v1".to_string();
    let reasoning_effort = session_configuration.collaboration_mode.reasoning_effort();

    session_configuration.provider.base_url = Some(runtime_base_url.clone());

    let (updated, provider_label) = session_configuration
        .apply(&SessionSettingsUpdate {
        ...
        .expect("gpt model update should preserve runtime provider override");

    assert_eq!(updated.provider_id, session_configuration.provider_id);
    assert_eq!(updated.provider.base_url, Some(runtime_base_url));
    assert_eq!(provider_label, None);
}

#[tokio::test]
async fn session_configuration_apply_restores_user_provider_after_family_auto_switch() {
    let mut session_configuration = make_session_configuration_for_tests().await;
    let user_base_url = "http://127.0.0.1:4020/v1".to_string();

    let mut original_config = (*session_configuration.original_config_do_not_use).clone();
    original_config.user_configured_provider.base_url = Some(user_base_url.clone());
    session_configuration.original_config_do_not_use = Arc::new(original_config);

    session_configuration.provider_id = "anthropic".to_string();
    session_configuration.provider = session_configuration
    ...
    let (updated, provider_label) = session_configuration
        ...
        .expect("gpt model update should restore explicit provider");
    ...
    assert_eq!(updated.provider.base_url, Some(user_base_url));
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    assert!(matches!(
        ...
        EventMsg::TurnComplete(TurnCompleteEvent {
            ...
            memory: None,}) if turn_id == tc.sub_id
    ));
}
