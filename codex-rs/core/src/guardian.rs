...
pub(crate) fn is_guardian_subagent_source(
    ...
    matches!(
        session_source,
        SessionSource::SubAgent(SubAgentSource::Other(name)) if name == GUARDIAN_SUBAGENT_NAME
    )
}
...
fn collect_guardian_transcript_entries(items: &[ResponseItem]) -> Vec<GuardianTranscriptEntry> {
    ...
    for item in items {
        let entry = match item {
            ...
            ResponseItem::CustomToolCallOutput { call_id, output } => non_empty_entry(
                GuardianTranscriptEntryKind::Tool(tool_names_by_call_id.get(call_id).map_or_else(
                    || "tool result".to_string(),
                    |name| format!("tool {name} result"),
                )),
                output.clone(),
...
async fn run_guardian_subagent(
    ...
    let codex = crate::codex_delegate::run_codex_thread_interactive(
        guardian_config,
...
fn format_guardian_action_pretty(action: &GuardianApprovalRequest) -> String {
    let value = truncate_guardian_action_value(guardian_approval_request_to_json(action));
