...
## Structured predicted effects
- Your final JSON must always include `predicted_effects`.
- Use only this first-phase effect taxonomy:
  - `protected_delete`
  - `protected_move_out`
  - `sensitive_read`
...
- For destructive or sensitive actions, `predicted_effects` must not be empty.
- Each predicted effect must use the most concrete scope you can justify from the evidence:
  - `target_path` for direct reads/deletes
  - `source_path` and `destination_path` for moves/transfers
  - `tool_name` and/or `process_name` for execution effects
  - `trusted_identity` when identity trust is part of the risk
- Set unused scope fields to `null`, not omitted.
- Set `recursive` to `true` only when the action genuinely needs descendant or directory-wide scope.
- `confidence` should reflect how certain you are that the planned action will have that effect.
