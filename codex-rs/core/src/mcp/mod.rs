    ...
    if connectors_enabled {
        ...
        servers
            ...
            .and_modify(|existing| {
                ...
                let normalized_base = normalize_codex_apps_base_url(&config.chatgpt_base_url);
                let default_urls = [
                    codex_apps_mcp_url_for_base_url(&config.chatgpt_base_url),
                    format!("{normalized_base}/apps"),
                    format!("{normalized_base}/api/codex/apps"),
                    format!("{normalized_base}/wham/apps"),
                ];
                let is_default_url = default_urls.iter().any(|url| existing_url == url.as_str());
