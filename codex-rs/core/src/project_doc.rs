...
fn render_js_repl_instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- `js_repl` may be provided as a freeform tool or a JSON tool. If freeform, send raw JavaScript text (do not wrap in JSON). If JSON, use the `code` property.\n");
