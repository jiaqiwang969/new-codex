...
mod tests {
    ...
    async fn snapshot_shell_does_not_inherit_stdin() -> Result<()> {
        ...
        // Use a non-login shell so we avoid executing user-specific login profiles (e.g. ~/.bash_profile),
        // which can be slow and introduce flaky timeouts. We still source `.bashrc` explicitly via
        // `bash_snapshot_script()` after setting HOME above, so stdin inheritance is still exercised.
        let output = run_script_with_timeout(&shell, &script, Duration::from_millis(500), false)
            .await
