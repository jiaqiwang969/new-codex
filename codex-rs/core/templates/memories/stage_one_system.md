...
4) `raw_memories/` (inputs distilled from Phase 1; this phase writes one per rollout)

In Phase 1, return exactly:
- `rawMemory` (detailed structured markdown evidence for consolidation)
...
Classify each task in `rawMemory` as one of:
...
============================================================
`rawMemory` FORMAT (STRICT STRUCTURE)
============================================================
...
Rules:
- Empty-field no-op must use empty strings for both keys.
- No additional keys.
...
3) Build `rawMemory` in the strict structure above.
