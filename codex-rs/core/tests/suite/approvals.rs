...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn smart_access_shell_emits_runtime_mismatch_when_daemon_blocks() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let daemon_log_path = PathBuf::from("/tmp/codex-es-daemon.log");
    ...
    let target_path = test.cwd.path().join("smart_access_runtime_mismatch.txt");
    let call_id = "smart-access-shell-runtime-mismatch";
    let command = format!(
        "printf '[Codex ES Daemon] Blocked protected delete: {}\\n' >> {} && exit 1",
        target_path.display(),
    ...
    let responses = mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ev_response_created("resp-smart-access-shell-runtime-mismatch-1"),
                event,
                ev_completed("resp-smart-access-shell-runtime-mismatch-1"),
            ]),
            sse(vec![
                ev_response_created("resp-smart-access-shell-runtime-mismatch-guardian"),
                ev_assistant_message(
                    "msg-smart-access-shell-runtime-mismatch-guardian",
                    &guardian_response,
                ),
                ev_completed("resp-smart-access-shell-runtime-mismatch-guardian"),
            ]),
            sse(vec![
                ev_response_created("resp-smart-access-shell-runtime-mismatch-2"),
                ev_assistant_message("msg-smart-access-shell-runtime-mismatch-2", "done"),
                ev_completed("resp-smart-access-shell-runtime-mismatch-2"),
            ]),
        ],
    )
    .await;
    ...
    assert!(
        statuses.contains(&GuardianAssessmentStatus::Approved),
        "expected Smart Access approval trace before runtime mismatch: {statuses:?}"
    );

    let runtime_mismatch = runtime_mismatch.expect("expected runtime mismatch trace");
    ...
    let expected_actual_effect = format!(
        "protected_delete:{}",
        target_path
            .canonicalize()
    ...
    assert_eq!(
        runtime_mismatch
            ...
            .and_then(Value::as_str),
        Some(expected_actual_effect.as_str())
    );

    let result = parse_result(&output_item_for_call(&responses, call_id));
    assert_eq!(result.exit_code, Some(1));
    assert!(
        result.stdout.contains("[SYSTEM SECURITY INTERVENTION]"),
        "expected Smart Access shell output to include endpoint security warning: {}",
        result.stdout
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn smart_access_shell_emits_runtime_mismatch_for_agentsmith_daemon_log() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let protected_dir = test.cwd.path().join("protected");
    fs::create_dir_all(&protected_dir)?;
    let target_path = protected_dir.join("smart_access_agentsmith_runtime_mismatch.txt");
    fs::write(&target_path, "keep me")?;
    let call_id = "smart-access-shell-agentsmith-runtime-mismatch";
    let command = format!(
    ...
    let responses = mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ev_response_created("resp-smart-access-shell-agentsmith-runtime-mismatch-1"),
                event,
                ev_completed("resp-smart-access-shell-agentsmith-runtime-mismatch-1"),
            ]),
            sse(vec![
                ev_response_created("resp-smart-access-shell-agentsmith-runtime-mismatch-guardian"),
                ev_assistant_message(
                    "msg-smart-access-shell-agentsmith-runtime-mismatch-guardian",
                    &guardian_response,
                ),
                ev_completed("resp-smart-access-shell-agentsmith-runtime-mismatch-guardian"),
            ]),
            sse(vec![
                ev_response_created("resp-smart-access-shell-agentsmith-runtime-mismatch-2"),
                ev_assistant_message(
                    "msg-smart-access-shell-agentsmith-runtime-mismatch-2",
                    "done",
                ),
                ev_completed("resp-smart-access-shell-agentsmith-runtime-mismatch-2"),
            ]),
        ],
    )
    .await;
    ...
    assert_eq!(result.exit_code, Some(1));
    assert!(
        result.stdout.contains("[AGENTSMITH DENIED]"),
        "expected shell output to include AgentSmith denial warning: {}",
        result.stdout
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(target_os = "linux")]
async fn smart_access_apply_patch_emits_runtime_mismatch_when_daemon_blocks() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let sandbox_policy_for_config = sandbox_policy.clone();
    let fake_codex_dir = TempDir::new()?;

    let fake_codex_path = fake_codex_dir.path().join("fake-codex");
    fs::write(
        &fake_codex_path,
        format!(
            "#!/bin/sh\npatch_input=\"$2\"\ntarget_path=$(printf '%s\\n' \"$patch_input\" | sed -n 's/^\\*\\*\\* Delete File: //p' | head -n 1)\nprintf '[Codex ES Daemon] Blocked protected delete: %s\\n' \"$target_path\" >> {daemon_log_path:?}\nexit 1\n"
        ),
    )?;
    let mut permissions = fs::metadata(&fake_codex_path)?.permissions();
    permissions.set_mode(0o755);
    fs::set_permissions(&fake_codex_path, permissions)?;

    let fake_codex_path_for_config = fake_codex_path.clone();
    let mut builder = test_codex()
        .with_model("gpt-5.1-codex")
        .with_config(move |config| {
            config.permissions.approval_policy = Constrained::allow_any(approval_policy);
            ...
            config.approvals_reviewer = ApprovalsReviewer::User;
            config.include_apply_patch_tool = true;
            config.codex_linux_sandbox_exe = Some(fake_codex_path_for_config);
        });
    let test = builder.build(&server).await?;
    ...
    let target_path = test
        .cwd
        .path()
        .join("smart_access_apply_patch_runtime_mismatch.txt");
    fs::write(&target_path, "before")?;

    let call_id = "smart-access-apply-patch-runtime-mismatch";
    let patch = format!(
        "*** Begin Patch\n*** Delete File: {}\n*** End Patch\n",
        target_path.display()
    );
    let guardian_response = json!({
        "risk_level": "low",
        "risk_score": 16,
        "rationale": "single-file delete inside the workspace root",
        "evidence": [],
        "predicted_effects": [{
            "kind": "protected_delete",
            "scope": {
                ...
                "process_name": "apply_patch",
                "trusted_identity": null,
                "recursive": false
            },
            "confidence": 97,
            "why": "The patch deletes one workspace file."
        }]
    })
    .to_string();
    ...
    let responses = mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ev_response_created("resp-smart-access-apply-patch-runtime-mismatch-1"),
                ev_apply_patch_function_call(call_id, &patch),
                ev_completed("resp-smart-access-apply-patch-runtime-mismatch-1"),
            ]),
            sse(vec![
                ev_response_created("resp-smart-access-apply-patch-runtime-mismatch-guardian"),
                ev_assistant_message(
                    "msg-smart-access-apply-patch-runtime-mismatch-guardian",
                    &guardian_response,
                ),
                ev_completed("resp-smart-access-apply-patch-runtime-mismatch-guardian"),
            ]),
            sse(vec![
                ev_response_created("resp-smart-access-apply-patch-runtime-mismatch-2"),
                ev_assistant_message("msg-smart-access-apply-patch-runtime-mismatch-2", "done"),
                ev_completed("resp-smart-access-apply-patch-runtime-mismatch-2"),
            ]),
        ],
    )
    .await;
    ...
    assert!(
        result.stdout.contains("[SYSTEM SECURITY INTERVENTION]"),
        "expected Smart Access apply_patch output to include endpoint security warning: {}",
        result.stdout
    );
