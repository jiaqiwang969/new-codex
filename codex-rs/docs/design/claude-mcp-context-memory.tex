...
本文从第一性原理出发，给出一套“\textbf{调用时上下文注入（Context Packet）} + \textbf{跨会话持久记忆（SQLite thread\_memory）}”的组合方案，并说明其与 Codex 官方方向（\texttt{get\_memory} 工具 + SQLite）如何对齐，以及如何验证/迭代。本文同时记录截至 \textbf{2026-02-10} 的落地实现现状与后续里程碑。
\end{abstract}
...
\subsection{指令文件统一：AGENTS.md 作为单一真相}
在多 Agent/多模型协作中，如果 Codex 依赖 \texttt{AGENTS.md}，而 Claude Code 依赖 \texttt{CLAUDE.md}，
就会形成“两套指令体系”的漂移风险。

建议将 \texttt{AGENTS.md} 作为单一真相来源（包含工程规范、约束、测试要求等），
并让 \texttt{CLAUDE.md} 仅承担“提示词工程/指针”角色：\textbf{引导 Claude 优先阅读并遵循 \texttt{AGENTS.md}}，
而不再维护一份独立且可能过期的长指令文件。

此外，我们在 Codex 侧的 \texttt{context} 注入已经会把 User instructions（含 \texttt{AGENTS.md}）一并传给 Claude，
这使得 Claude 即使不主动读文件，也能获取同一套约束。
...
\begin{enumerate}[nosep]
  ...
  \item Agent Worktrees / Resume-Clone：Fork 会话与 Spawned Agent 可选使用隔离 git worktree，并将 thread\_id \textrightarrow worktree 的映射持久化为 lease；resume 进入历史会话时自动切回对应 worktree，且在 worktree 缺失时可自动重建。
\end{enumerate}
...
\section{Agent Worktrees / Resume-Clone：多 Agent 协作的工作区隔离}
\subsection{问题：共享工作区会导致协作互相污染}
当 Codex 同时驱动多个 Agent（例如：主线程 + Claude MCP + Spawned Agent），如果它们共享同一个工作区：
\begin{itemize}[nosep]
  \item 改动会互相覆盖：不同 agent 的未提交变更混在一起，难以归因与回滚；
  \item 进入历史 session 时，\textbf{“正确的工作目录/分支上下文”无法复原}：即使 transcript 能 resume，代码视角也可能已经不在当时的 checkout。
\end{itemize}
从第一性原理看：\textbf{线程（thread）不仅是对话状态，也是“执行环境状态”的边界}；因此需要隔离执行环境。

\subsection{解决：每个 thread 一个 worktree + lease 映射}
采用 git worktree 实现轻量隔离（非完整 clone），对每个 Forked Session / Spawned Agent：
\begin{itemize}[nosep]
  \item 创建 worktree 路径：\texttt{<repo>/.codex/worktrees/\{fork|agent\}/<uuid>}；
  \item 创建并绑定分支：\texttt{codex/\{fork|agent\}/<uuid>}；
  \item 写入 lease：\texttt{<repo>/.codex/leases/<thread\_id>.json}，记录：
    \begin{itemize}[nosep]
      \item thread\_id、parent\_thread\_id；
      \item worktree\_path、branch、purpose；
      \item created\_at/updated\_at、pid 等元信息。
    \end{itemize}
\end{itemize}
这样，Codex 可以把“对话线程”与“代码工作区”做稳定绑定。

\subsection{恢复：resume 时切回 worktree；缺失则自动重建}
当用户在 SessionBar 中 resume 进入历史 thread 时：
\begin{itemize}[nosep]
  \item 若存在 lease：将 thread 的 cwd 切换到 \texttt{lease.worktree\_path}；
  \item 若 worktree 目录已被清理/丢失：执行 \texttt{git worktree prune} + \texttt{git worktree add --force <path> <branch>} 复原；
  \item 复原后再从 rollout 重建 transcript（使“代码上下文 + 对话上下文”一致）。
\end{itemize}

\subsection{调试工具（Debug CLI）}
为支持排障与人工恢复，增加 debug 命令：
\begin{lstlisting}
codex debug agent-worktrees list [--json]
...
\begin{enumerate}[nosep]
  ...
  \item M4（已具备工具，但仍需产品化）：已存在 debug CLI 用于回填历史线程记忆：
    \begin{lstlisting}
codex debug thread-memory backfill --all --archived --force
    \end{lstlisting}
    后续建议补齐更友好的文档、运行前检查与 dry-run/限流策略。
