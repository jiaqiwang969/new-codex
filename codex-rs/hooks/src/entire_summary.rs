...
pub async fn save_summary(
    ...
    fs::create_dir_all(&summaries_dir)
        .await
        .map_err(|err| format!("Failed to create .entire/summaries directory: {err}"))?;
    ...
    fs::write(&summary_path, summary_json)
        .await
        .map_err(|err| format!("Failed to write summary file: {err}"))?;
...
pub async fn load_summary(
    repo_root: &Path,
    checkpoint_id: &str,
) -> Result<Option<EntireSummary>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let content = fs::read_to_string(&summary_path)
        .await
        .map_err(|err| format!("Failed to read summary file: {err}"))?;

    let summary: EntireSummary = serde_json::from_str(&content)
        .map_err(|err| format!("Failed to parse summary JSON: {err}"))?;
...
mod tests {
    ...
    #[test]
    fn build_why_prompt_includes_key_sections() {
    ...
    #[test]
    fn truncate_response_marks_large_payloads() {
    ...
    #[tokio::test]
    async fn save_and_load_summary_round_trip() {
        let repo_root = tempfile::tempdir().expect("create temp dir");
        let summary = EntireSummary {
            is_meaningful: true,
            motivation: Some("Preserve custom context".to_string()),
            approach: Some("Persist a structured WHY summary".to_string()),
            challenges: None,
            tradeoffs: Some("Slight extra disk usage".to_string()),
            outcome: Some("Future sessions can reuse prior rationale".to_string()),
        };
