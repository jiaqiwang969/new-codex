...
impl CodexAuth {
    ...
    #[doc(hidden)]
    pub fn from_api_key_and_env_keys_for_testing(
        api_key: &str,
        api_keys_by_env_var: HashMap<String, String>,
    ) -> Self {
        Self::ApiKey(ApiKeyAuth {
...
const AUTH_POOL_JSON_FILE: &str = "auth-pool.json";
...
fn load_pool_api_keys(codex_home: &Path) -> HashMap<String, String> {
    let pool_path = codex_home.join(AUTH_POOL_JSON_FILE);
    ...
    let raw: HashMap<String, serde_json::Value> = match serde_json::from_str(&contents) {
        Ok(m) => m,
        Err(e) => {
            tracing::warn!("failed to parse {AUTH_POOL_JSON_FILE}: {e}");
            return HashMap::new();
        }
    };
    raw.into_iter()
