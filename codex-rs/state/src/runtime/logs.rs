...
mod tests {
    ...
    #[tokio::test]
    async fn insert_logs_writes_to_dedicated_logs_database() {
        ...
        let logs_pool = SqlitePoolOptions::new()
            .max_connections(1)
            .connect_with(
                SqliteConnectOptions::new()
                    .filename(crate::logs_db_path(codex_home.as_path()))
                    .log_statements(log::LevelFilter::Off),
            )
            .await
            .expect("open logs db");

        let state_count = query_scalar::<_, i64>("SELECT COUNT(*) FROM logs")
            .fetch_one(&state_pool)
            .await
            .expect("query state logs count");
        let logs_count = query_scalar::<_, i64>("SELECT COUNT(*) FROM logs")
            .fetch_one(&logs_pool)
            .await
            .expect("query dedicated logs count");
