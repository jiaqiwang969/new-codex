...
impl App {
    ...
    pub async fn run(
            ...
            SessionSelection::Fork(target_session) => {
                ...
                if config.features.enabled(Feature::AgentWorktrees) {
                    ...
                    if let Some(worktree) = fork_worktree.as_ref() {
                        config.cwd = AbsolutePathBuf::try_from(worktree.path.clone())
                            .wrap_err("fork worktree path must be absolute")?;
                    }
                }
        ...
        let mut app = Self {
            ...
            session_bar: SessionBar::new(session_bar_cwd.to_path_buf(), session_bar_home),
    ...
    async fn resume_session_from_path(
        ...
        .await?
        {
            ...
            crate::ResolveCwdOutcome::Continue(None) => current_cwd.to_path_buf(),
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::ResumeSession(target_session) => {
                self.resume_session_from_path(tui, target_session, /*allow_cwd_prompt*/ true)
                    .await?;
                tui.frame_requester().schedule_frame();
            }
    ...
    fn session_bar_enter_event(&self) -> AppEvent {
        ...
        if let Some(session) = self.session_bar.selected_session() {
            let thread_id = ThreadId::from_string(&session.id).unwrap_or_default();
            return AppEvent::ResumeSession(SessionTarget {
    ...
    async fn handle_session_bar_key(&mut self, tui: &mut tui::Tui, key_event: KeyEvent) {
        ...
        match key_event {
            ...
            | KeyEvent {
                ...
                self.session_bar.set_focus(/*focused*/ false);
                tui.frame_requester().schedule_frame();
            }
        ...
        match key_event {
            ...
            KeyEvent {
                ...
                if self.panel_focus == PanelFocus::Chat {
                    self.panel_focus = PanelFocus::Sessions;
                    self.session_bar.set_focus(/*focused*/ true);
...
"#;
    ...
    async fn render_clear_ui_header_after_long_transcript_for_snapshot() -> String {
        ...
        app.chat_widget
            .set_model("gpt-test", /*provider_label*/ None);
        app.chat_widget
            .set_reasoning_effort(Some(ReasoningEffortConfig::High));
    ...
    async fn clear_ui_header_shows_fast_status_only_for_gpt54() {
        ...
        app.config.cwd = PathBuf::from("/tmp/project").abs();
        app.chat_widget
            .set_model("gpt-5.4", /*provider_label*/ None);
        app.chat_widget
    ...
    async fn make_test_app() -> App {
        ...
        App {
            ...
            primary_session_configured: None,
            pending_primary_events: VecDeque::new(),
            session_bar: SessionBar::new(sb_cwd.to_path_buf(), sb_home),
            panel_focus: PanelFocus::Chat,
        }
    }
    ...
    async fn session_bar_enter_event_uses_resume_for_selected_history() {
        ...
        match app.session_bar_enter_event() {
            AppEvent::ResumeSession(target_session) => {
                assert_eq!(target_session.path, session_path)
            }
    ...
    async fn make_test_app_with_channels() -> (
        ...
        (
            App {
                ...
                primary_session_configured: None,
                pending_primary_events: VecDeque::new(),
                session_bar: SessionBar::new(sb_cwd2.to_path_buf(), sb_home2),
                panel_focus: PanelFocus::Chat,
            },
            rx,
            op_rx,
