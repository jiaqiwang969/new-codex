...
"
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::RefImage => {
                self.handle_ref_image_command(/*args*/ None);
            }
            SlashCommand::RefImageBatch => {
                self.handle_ref_image_batch_command(/*args*/ None);
            }
            SlashCommand::ImageQuality => {
                self.handle_image_quality_command(/*args*/ None);
            }
            SlashCommand::AspectRatio => {
                self.handle_aspect_ratio_command(/*args*/ None);
            }
            SlashCommand::ClearRef => {
                self.handle_clear_ref_command();
            }
            SlashCommand::PdfUpdate => {
                self.handle_pdf_update_command(/*args*/ None);
            }
        ...
        match cmd {
            ...
            SlashCommand::ModelSub if !trimmed.is_empty() => {
                ...
                let normalized_selector =
                    trimmed.trim().to_ascii_lowercase().replace([' ', '_'], "-");
                ...
                if available_models.iter().any(|model| model == trimmed) {
                    ...
                    self.add_error_message(
                        "Usage: /model-sub <inherit|clear|model-slug>".to_string(),
                    );
                }
                self.bottom_pane.drain_pending_submission_state();
            }
    ...
    fn handle_ref_image_command(&mut self, args: Option<String>) {
        ...
        if trimmed.eq_ignore_ascii_case("clear") {
            ...
            self.add_info_message("Reference images cleared.".to_string(), /*hint*/ None);
            return;
        }
        ...
        if final_paths.is_empty() && prompt.is_none() {
            self.add_info_message("No paths provided.".to_string(), /*hint*/ None);
            return;
        }
        ...
        match ratio_arg {
            ...
            Some(ratio) => {
                if matches!(ratio.as_str(), "1:1" | "16:9" | "9:16" | "4:3" | "3:4") {
                    ...
                    self.add_info_message(
                        format!("Aspect ratio set to {ratio}"),
                        /*hint*/ None,
                    );
                } else {
                    self.add_info_message(
    ...
    fn handle_ref_image_batch_command(&mut self, args: Option<String>) {
        ...
        if trimmed.is_empty() || trimmed.eq_ignore_ascii_case("help") {
            ...
            self.add_info_message(help.to_string(), /*hint*/ None);
            return;
        }
    ...
    fn process_next_batch_image(&mut self) {
        ...
        if let Some((completion_msg, pdf_info)) = batch_complete_info {
            ...
            self.add_info_message(completion_msg, /*hint*/ None);
        ...
        {
            self.add_info_message(progress_msg, /*hint*/ None);
