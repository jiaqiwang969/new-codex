...
## Fresh Upstream Baseline Validation (2026-03-06)

Validated in `~/.config/superpowers/worktrees/new-codex/integration-upstream-main-b2/codex-rs` against `upstream/main` at `b6d43ec8e`.

- `cargo test -p codex-core` — not clean in this environment (`1433 passed; 4 failed; 5 ignored`)
- `cargo test -p codex-tui` — clean (`1240 passed; 0 failed; 2 ignored`)
- `cargo test -p codex-app-server` — clean (`212 passed; 0 failed; 1 ignored`)

Observed `codex-core` baseline failures:
- `seatbelt::tests::create_seatbelt_args_with_read_only_git_pointer_file` — filesystem permission failure while creating `.git` pointer file
- `seatbelt::tests::create_seatbelt_args_with_read_only_git_and_codex_subpaths` — filesystem permission failure while creating `.codex/config.toml`
- `shell::tests::detects_zsh` — environment-specific zsh path mismatch (`/run/current-system/sw/bin/zsh` vs `/bin/zsh`)
- `shell::tests::fish_fallback_to_zsh` — environment-specific zsh path mismatch (`/run/current-system/sw/bin/zsh` vs `/bin/zsh`)

These failures appear upstream-baseline and should not be conflated with custom migration regressions.
